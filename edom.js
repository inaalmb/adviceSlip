...
function validateFormSrpskiPridruzi(e) {
  ...
  let namePattern = /^[A-Z][a-z]{6,}$/;
  let surnamePattern = /^[A-Z][a-z]{6,}$/;
  let emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  let passwordPattern = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).$/;

  // validate all of this
  if (!namePattern.test(name)) {
    nameMessage.innerHTML = "Name is not valid.";
  ...
  if (!surnamePattern.test(surname)) {
    surnameMessage.innerHTML = "Surname is not valid..";
  ...
  if (!passwordPattern.test(password)) {
    passwordMessage.innerHTML = "Password is not valid.";
